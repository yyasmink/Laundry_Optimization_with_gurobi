# -*- coding: utf-8 -*-
"""
Created on Tue May 12 21:56:04 2026

@author: yasmin
"""
# MTZ ile 2-CF deki her şey aynı sadece 2-cf yerine MTZ VAR 
...
# ==========================================
# 3. ANA MODEL (MTZ VERSİYONU)
# ==========================================
def solve_daily_vrp(gun):
    print(f"\n{'='*60}")
    print(f">>> {gun.upper()} — LIFTED MTZ Formulation (A/B Test)")
    print(f"{'='*60}")
    ...
    # ---> 2CF'DENGELENEN 'f' DEĞİŞKENİ ÇIKARILDI, YERİNE MTZ 'u' EKLENDİ <---
    u = model.addVars(H_aktif, K, vtype=GRB.CONTINUOUS, lb=1, ub=n, name="u")
    ...
    # =================================================================
    # ---> MTZ ALT TUR ELEME KISITLARI (2CF YERİNE) <---
    # =================================================================
    for k in K:
        for i in H_aktif:
            for j in H_aktif:
                if i != j:
                    model.addConstr(
                        u[i, k] - u[j, k] + n * x[i, j, k] + (n - 2) * x[j, i, k] <= n - 1, 
                        f"Lifted_MTZ_{i}_{j}_{k}"
                    )

    # =================================================================
    # ZAMAN AKIŞI KISITLARI (Dinamik Big-M ile tamamen aynı tutuldu)
    # =================================================================
    for k in K:
        for j in H_aktif:
            model.addConstr(
    ...
    # =================================================================
    # GÜÇLENDİRİCİ KISITLAR (Sparsification vb. tamamen aynı)
    # =================================================================
    # 2-cycle elimination
    for k in K:
        for i in H_aktif:
    ...
    # =================================================================
    # GUROBI PARAMETRELERİ (Tamamen aynı)
    # =================================================================
...
# ==========================================
print("\n" + "🚀"*15)
print("HAFTALIK PERİYODİK ÇÖZÜM BAŞLATILIYOR (MTZ TESTİ)")
print("🚀"*15)

# Tüm haftayı beklemek istemezsen sadece Pazar veya Pazartesiyi çalıştırabilirsin.
# Şimdilik tüm haftayı çalıştıracak döngü:
for gun in D:
